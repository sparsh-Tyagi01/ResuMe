...
const JakeTemplate: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div
      className="w-[794px] min-h-[1123px] bg-white text-black p-8 box-border text-[12px] leading-[1.4] relative print-preview-canvas"
      style={{ fontFamily: "'EB Garamond', serif" }}
    >
      {/* Header */}
      <div className="text-center mb-4">
        <h1 className="text-2xl font-bold tracking-wide uppercase">{personalInfo.name || "YOUR NAME"}</h1>
        <div className="text-xs text-slate-700 flex flex-wrap justify-center gap-1.5 mt-1">
        ...
        {personalInfo.title && (
          <p className="text-sm font-semibold tracking-wider text-slate-800 uppercase mt-0.5">
            {personalInfo.title}
          </p>
        )}
      </div>
      ...
      {summary && (
        <div className="mb-3.5">
          <h2 className="text-[13px] font-bold uppercase tracking-wider border-b border-black pb-0.5 mb-1.5">
      ...
      {education && education.length > 0 && (
        <div className="mb-3.5">
          ...
          {education.map((edu) => (
            <div key={edu.id} className="mb-1.5 text-slate-800">
      ...
      {experience && experience.length > 0 && (
        <div className="mb-3.5">
          ...
          {experience.map((exp) => (
            <div key={exp.id} className="mb-2 text-slate-800">
              <div className="flex justify-between font-bold">
      ...
      {projects && projects.length > 0 && (
        <div className="mb-3.5">
          ...
          {projects.map((proj) => (
            <div key={proj.id} className="mb-2 text-slate-800">
              <div className="flex justify-between font-bold">
                <span>
      ...
      {leadership && leadership.length > 0 && (
        <div className="mb-3.5">
          ...
          {leadership.map((lead) => (
            <div key={lead.id} className="mb-2 text-slate-800">
              <div className="flex justify-between font-bold">
                <span>{lead.role}</span>
